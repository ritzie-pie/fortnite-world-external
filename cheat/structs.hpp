...
template <typename T>
inline extern bool read_memory_g(uint64_t addr, T* out, size_t len);
inline extern int screen_width;
inline extern int screen_height;
class vector2
{
public:
	vector2() : x(0.f), y(0.f)
...
public:
	...
	[[nodiscard]]
	std::vector<T> getiter(size_t maxCount = 30000) const {
		if (m_nCount > maxCount)
			return {};

		std::vector<T> buffer(m_nCount);
		read_memory_g(m_Data, buffer.data(), m_nCount);
		if (buffer.data()) {
			return buffer;
		}
		else {
