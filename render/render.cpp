...
{
	if (ImGui_ImplWin32_WndProcHandler(hWnd, pMsg, wParam, lParam) && menu.opened)
		return TRUE;
...
void capFPS() {
	...
	static auto lastFrameTime = steady_clock::now();
	...
	while (elapsedTime < targetFrameDuration) {
		...
		elapsedTime = duration<double>(currentTime - lastFrameTime).count();
	}
